...
parser = argparse.ArgumentParser(description='Script to get coins from a faucet.', epilog='You may need to start with double-dash (--) when providing bitcoinII-cli arguments.')
parser.add_argument('-c', '--cmd', dest='cmd', default='bitcoinII-cli', help='bitcoinII-cli command to use')
...
parser.add_argument('-a', '--addr', dest='addr', default='', help='BitcoinII address to which the faucet should send')
...
parser.add_argument('bitcoinII_cli_args', nargs='*', help='Arguments to pass on to bitcoinII-cli (default: -signet)')

args = parser.parse_args()

if args.bitcoinII_cli_args == []:
    args.bitcoinII_cli_args = ['-signet']


def bitcoinII_cli(rpc_command_and_params):
    argv = [args.cmd] + args.bitcoinII_cli_args + rpc_command_and_params
    try:
...
if args.faucet.lower() == DEFAULT_GLOBAL_FAUCET:
    ...
    curr_signet_hash = bitcoinII_cli(['getblockhash', '1'])
...
if args.addr == '':
    ...
    args.addr = bitcoinII_cli(['getnewaddress', 'faucet', 'bech32'])
