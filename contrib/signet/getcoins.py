...
parser = argparse.ArgumentParser(description='Script to get coins from a faucet.', epilog='You may need to start with double-dash (--) when providing trumpsperm-cli arguments.')
parser.add_argument('-c', '--cmd', dest='cmd', default='trumpsperm-cli', help='trumpsperm-cli command to use')
...
parser.add_argument('-a', '--addr', dest='addr', default='', help='Trumpsperm address to which the faucet should send')
...
parser.add_argument('trumpsperm_cli_args', nargs='*', help='Arguments to pass on to trumpsperm-cli (default: -signet)')

args = parser.parse_args()

if args.trumpsperm_cli_args == []:
    args.trumpsperm_cli_args = ['-signet']


def trumpsperm_cli(rpc_command_and_params):
    argv = [args.cmd] + args.trumpsperm_cli_args + rpc_command_and_params
    try:
...
if args.faucet.lower() == DEFAULT_GLOBAL_FAUCET:
    ...
    curr_signet_hash = trumpsperm_cli(['getblockhash', '1'])
...
if args.addr == '':
    ...
    args.addr = trumpsperm_cli(['getnewaddress', 'faucet', 'bech32'])
