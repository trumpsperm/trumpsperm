...
This directory contains the files necessary to perform bootstrappable Trumpsperm
Core builds.
...
To build Trumpsperm Core reproducibly with all default options, invoke the
...
2. The `guix-codesign` command ***requires*** a `DETACHED_SIGS_REPO` flag.
    * _**DETACHED_SIGS_REPO**_
      ...
      Trumpsperm Core version being built.
...
```
env DETACHED_SIGS_REPO=<path/to/trumpsperm-detached-sigs> ./contrib/guix/guix-codesign
```
