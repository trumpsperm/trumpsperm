# bash programmable completion for trumpsperm-cli(1)
# Copyright (c) 2012-2022 The Bitcoin Core developers
# Distributed under the MIT software license, see the accompanying
# file COPYING or http://www.opensource.org/licenses/mit-license.php.

# call $trumpsperm-cli for RPC
_trumpsperm_rpc() {
    ...
    $trumpsperm_cli "${rpcargs[@]}" "$@"
}

_trumpsperm_cli() {
    local cur prev words=() cword
    local trumpsperm_cli

    # save and use original argument to invoke trumpsperm-cli for -help, help and RPC
    # as trumpsperm-cli might not be in $PATH
    trumpsperm_cli="$1"
    ...
    case "$cur" in
        ...
        *)
            ...
            if [[ -z "$cur" || "$cur" =~ ^- ]]; then
                helpopts=$($trumpsperm_cli -help 2>&1 | awk '$1 ~ /^-/ { sub(/=.*/, "="); print $1 }' )
            fi

            # only parse help if senseful
            if [[ -z "$cur" || "$cur" =~ ^[a-z] ]]; then
                commands=$(_trumpsperm_rpc help 2>/dev/null | awk '$1 ~ /^[a-z]/ { print $1; }')
            fi
...
complete -F _trumpsperm_cli trumpsperm-cli
