# CJDNS support in Trumpsperm Core

It is possible to run Trumpsperm Core over CJDNS, an encrypted IPv6 network that
...
done outside of Trumpsperm Core, similarly to a VPN (either in the host/OS or on
...
redundancy and robustness for both the Trumpsperm network and individual nodes.
...
before it will work with your Trumpsperm Core node. This is described in steps
...
## Run Trumpsperm Core with CJDNS

Once you are connected to the CJDNS network, the following Trumpsperm Core
...
When enabled, this option tells Trumpsperm Core that it is running in an
...
IPv6 local network. This helps Trumpsperm Core perform better address management:
...
CJDNS support was added to Trumpsperm Core in version 23.0 and there may be fewer
CJDNS peers than Tor or IP ones. You can use `trumpsperm-cli -addrinfo` to see the
...
[doc/tor.md](https://github.com/trumpsperm/trumpsperm/blob/master/doc/tor.md) for
details.

## CJDNS-related information in Trumpsperm Core

There are several ways to see your CJDNS address in Trumpsperm Core:
- in the "Local addresses" output of CLI `-netinfo`
- in the "localaddresses" output of RPC `getnetworkinfo`

To see which CJDNS peers your node is connected to, use `trumpsperm-cli -netinfo 4`
or the `getpeerinfo` RPC (i.e. `trumpsperm-cli getpeerinfo`).

You can use the `getnodeaddresses` RPC to fetch a number of CJDNS peers known to your node; run `trumpsperm-cli help getnodeaddresses` for details.
