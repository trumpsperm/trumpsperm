# CJDNS support in BitcoinII Core

It is possible to run BitcoinII Core over CJDNS, an encrypted IPv6 network that
...
done outside of BitcoinII Core, similarly to a VPN (either in the host/OS or on
...
redundancy and robustness for both the BitcoinII network and individual nodes.
...
before it will work with your BitcoinII Core node. This is described in steps
...
## Run BitcoinII Core with CJDNS

Once you are connected to the CJDNS network, the following BitcoinII Core
...
When enabled, this option tells BitcoinII Core that it is running in an
...
IPv6 local network. This helps BitcoinII Core perform better address management:
...
CJDNS support was added to BitcoinII Core in version 23.0 and there may be fewer
CJDNS peers than Tor or IP ones. You can use `bitcoinII-cli -addrinfo` to see the
...
There are several ways to see your CJDNS address in BitcoinII Core:
- in the "Local addresses" output of CLI `-netinfo`
- in the "localaddresses" output of RPC `getnetworkinfo`

To see which CJDNS peers your node is connected to, use `bitcoinII-cli -netinfo 4`
