# I2P support in BitcoinII Core

It is possible to run BitcoinII Core as an
...
## Run BitcoinII Core with an I2P router (proxy)

A running I2P router (proxy) is required with the [SAM](https://geti2p.net/en/docs/api/samv3)
application bridge enabled. The following routers are recommended for use with BitcoinII Core:
...
Once an I2P router with SAM enabled is up and running, use the following BitcoinII
...
debug log about your I2P configuration and connections. Run `bitcoinII-cli help
...
The first time BitcoinII Core connects to the I2P router, it automatically
...
`i2p_private_key` in the BitcoinII Core data directory.  The persistent I2P
...
## Fetching I2P-related information from BitcoinII Core

There are several ways to see your I2P address in BitcoinII Core if accepting
incoming I2P connections (`-i2pacceptincoming`):
- in the "Local addresses" output of CLI `-netinfo`
- in the "localaddresses" output of RPC `getnetworkinfo`
- in the debug log (grep for `AddLocal`; the I2P address ends in `.b32.i2p`)

To see which I2P peers your node is connected to, use `bitcoinII-cli -netinfo 4`
or the `getpeerinfo` RPC (e.g. `bitcoinII-cli getpeerinfo`).

You can use the `getnodeaddresses` RPC to fetch a number of I2P peers known to your node; run `bitcoinII-cli help getnodeaddresses` for details.
...
To allow future upgrades to newer versions of SAM, BitcoinII Core sets its
listening port to 0 when listening for incoming I2P connections and advertises
its own I2P address with port 0. Furthermore, it will not attempt to connect to
I2P addresses with a non-zero port number because with SAM v3.1 the destination
port (`TO_PORT`) is always set to 0 and is not in the control of BitcoinII Core.
...
It's important that the nodes of a popular application like BitcoinII contribute
...
## Bundling I2P in a BitcoinII application
