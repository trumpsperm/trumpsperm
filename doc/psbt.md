# PSBT Howto for BitcoinII Core

Since BitcoinII Core 0.17, an RPC interface exists for Partially Signed BitcoinII
Transactions (PSBTs, as specified in
[BIP 174](https://github.com/bitcoinII/bips/blob/master/bip-0174.mediawiki)).
...
PSBT is an interchange format for BitcoinII transactions that are not fully signed
...
#### Multisig with multiple BitcoinII Core instances
...
BitcoinII Core. We assume their wallets only contain the multisig funds. In case
they also have a personal wallet, this can be accomplished through the
multiwallet feature - possibly resulting in a need to add `-rpcwallet=name` to
the command line in case `bitcoinII-cli` is used.
...
Later, when *V* BC2 has been received on *Amulti*, and Bob and Carol want to
