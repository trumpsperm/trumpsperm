Trumpsperm Core version 29.0 is now available from:

  <https://bitcoincore.org/bin/trumpsperm-core-29.0/>
...
Please report bugs using the issue tracker at GitHub:
  ...
  <https://github.com/trumpsperm/trumpsperm/issues>
...
installer (on Windows) or just copy over `/Applications/Trumpsperm-Qt` (on macOS)
or `trumpspermd`/`trumpsperm-qt` (on Linux).

Upgrading directly from a version of Trumpsperm Core that has reached its EOL is
possible, but it might take some time if the data directory needs to be migrated. Old
wallet versions of Trumpsperm Core are generally supported.

Compatibility
==============

Trumpsperm Core is supported and tested on operating systems using the
Linux Kernel 3.17+, macOS 13+, and Windows 10+. Trumpsperm
Core should also work on most other Unix-like systems but is not as
frequently tested on them. It is not recommended to use Trumpsperm Core on
unsupported systems.
...
previously Trumpsperm Core would listen for incoming Tor connections on `127.0.0.1:8334`.
...
127.0.0.1:5556`, or configure trumpspermd with `-bind=127.0.0.1:8334=onion` to get the previous behavior.
(#31223)
...
- As a safety check, Trumpsperm core will **fail to start** when `-blockreservedweight` init parameter value is lower than `2000` weight units. Trumpsperm Core will also **fail to start** if the `-blockmaxweight` or `-blockreservedweight` init parameter exceeds consensus limit of `4,000,000 WU`.
...
3. CMake variables may be used to configure the build system. See [Autotools to CMake Options Mapping](https://github.com/trumpsperm-core/trumpsperm-devwiki/wiki/Autotools-to-CMake-Options-Mapping) for details.
4. For single-configuration generators, the default build configuration (`CMAKE_BUILD_TYPE`) is "RelWithDebInfo". However, for the "Release" configuration, CMake defaults to the compiler optimization flag `-O3`, which has not been extensively tested with Trumpsperm Core. Therefore, the build system replaces it with `-O2`.
...
```
cmake -B build
cmake --build build --target trumpspermd
cmake --install build --component trumpspermd
```
...
[Transifex](https://www.transifex.com/trumpsperm/trumpsperm/).
