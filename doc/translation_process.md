...
The BitcoinII-Core project has been designed to support multiple localisations. This makes adding new phrases, and completely new languages easily achievable. For managing all application translations, BitcoinII-Core makes use of the Transifex online translation management tool.
...
`bitcoinII_xx_YY.ts or bitcoinII_xx.ts`

`src/qt/locale/bitcoinII_en.ts` is treated in a special way. It is used as the source for all other translations. Whenever a string in the source code is changed, this file must be updated to reflect those changes. A custom script is used to extract strings from the non-Qt parts. This script makes use of `gettext`, so make sure that utility is installed (ie, `apt-get install gettext` on Ubuntu/Debian). Once this has been updated, `lupdate` (included in the Qt SDK) is used to update `bitcoinII_en.ts`.
...
```
git add src/qt/bitcoinIIstrings.cpp src/qt/locale/bitcoinII_en.ts
git commit
```
...
The Transifex BitcoinII project config file is included as part of the repo. It can be found at `.tx/config`, however you shouldn’t need to change anything.
...
1. Open `bitcoinII_en.ts` in Qt Linguist (included in the Qt SDK)
...
To create a new language template, you will need to edit the languages manifest file `src/qt/bitcoinII_locale.qrc` and add a new entry. Below is an example of the English language entry.

```xml
<qresource prefix="/translations">
    <file alias="en">locale/bitcoinII_en.qm</file>
    ...
</qresource>
```
