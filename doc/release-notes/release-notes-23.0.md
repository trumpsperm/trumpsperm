...
BitcoinII Core release binaries for Linux now include experimental tracepoints which
...
* On startup, the list of banned hosts and networks (via `setban` RPC) in
  `banlist.dat` is ignored and only `banlist.json` is considered. BitcoinII Core
...
- CLI `-addrinfo` now returns a single field for the number of `onion` addresses
  ...
  for Tor V2 addresses was removed from BitcoinII Core in 22.0. (#22544)
