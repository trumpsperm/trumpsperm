...
BitcoinII Core is supported and extensively tested on operating systems using
the Linux kernel, macOS 10.10+, and Windows 7 and newer. It is not recommended
to use BitcoinII Core on unsupported systems.
...
to open the BitcoinII Core .dmg. This is due to new signing requirements
imposed by Apple, which the BitcoinII Core project does not yet adhere too.
...
- `-blockfilterindex` enables the creation of BIP158 block filters for
  ...
  currently use about 4 GiB of space.  Note: this version of BitcoinII
...
- In 0.18.0, a `./configure` flag was introduced to allow disabling BIP70 support in the GUI (support was enabled by default). In 0.19.0, this flag is now __disabled__ by default. If you want to compile BitcoinII Core with BIP70 support in the GUI, you can pass `--enable-bip70` to `./configure`. (#15584)
...
- `generate` is now removed after being deprecated in BitcoinII Core 0.18.
...
- BIP 61 reject messages were deprecated in v0.18. They are now disabled
  ...
  future version of BitcoinII Core. (#14054)

- To eliminate well-known denial-of-service vectors in BitcoinII Core,
  ...
  This prevents BitcoinII Core from sending the BIP111 NODE_BLOOM service
  ...
  of BitcoinII Core and nodes that have manually enabled BIP37 support,
...
- `getblockchaininfo` no longer returns a `bip9_softforks` object.
  ...
  an additional `type` field describes how BitcoinII Core determines
...
- When fetching a transaction announced by multiple peers, previous versions of
  BitcoinII Core would sequentially attempt to download the transaction from each
...
- If a Tor hidden service is being used, BitcoinII Core will be bound to
...
- Transactions with outputs paying v1 to v16 witness versions (future
  ...
  and services can accept any valid bech32 BitcoinII address without
...
- A new wallet flag `avoid_reuse` has been added (default off). When
  ...
  partial spends" (added in BitcoinII Core v0.17.0), this can eliminate a
...
- #15649 Add ChaCha20Poly1305@BitcoinII AEAD (jonasschnelli)
...
- #15639 bitcoinII-wallet tool: Drop libbitcoinII_server.a dependency (ryanofsky)
...
- #16521 Use the default maxfeerate value as BC2/kB (Remagpie)
...
- #15239 scripts and tools: Move non-linux build source tarballs to "bitcoinII-binaries/version" directory (hebasto)
...
- #16441 remove qt libjpeg check from bitcoinII_qt.m4 (fanquake)
...
- #16750 msbuild: adds bench_bitcoinII to auto generated project files (sipsorcery)
...
- #15773 Add BitcoinIITestFramework::sync_* methods (MarcoFalke)
...
- #15352 Reduce noise level in test_bitcoinII output (practicalswift)
...
- #15896 feature_filelock, interface_bitcoinII_cli: Use PACKAGE_NAME in messages rather than hardcoding BitcoinII Core (luke-jr)
...
- #16277 Suppress output in test_bitcoinII for expected errors (gertjaap)
...
- #16929 follow-up to rpc: default maxfeerate value as BC2/kB (jonatack)
...
- #17449 fix uninitialized variable nMinerConfirmationWindow (bitcoinIIVBR)

### Documentation
- #15514 Update Transifex links (fanquake)
- #15513 add "sections" info to example bitcoinII.conf (fanquake)
...
- #16080 build/doc: update bitcoinII_config.h packages, release process (jonatack)
...
- #16448 add note on precedence of options in bitcoinII.conf (fanquake)
...
- #16621 add default bitcoinII.conf locations (GChuf)
...
- #16863 Add a missing closing parenthesis in the bitcoinII-wallet's help (darosior)
- #16757 CChainState return values (MarcoFalke)
- #16847 add comments clarifying how local services are advertised (jamesob)
- #16812 Fix whitespace errs in .md files, bitcoinII.conf, and Info.plist.in (ch4ot1c)
