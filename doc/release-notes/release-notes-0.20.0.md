...
- Instead of running `make` simply, you should instead run
  `BITCOINII_GENBUILD_NO_GIT=1 make`.
...
#### Removal of BIP61 reject network messages from BitcoinII Core

The `-enablebip61` command line option to enable BIP61 has been removed.
(#17004)

This feature has been disabled by default since BitcoinII Core version 0.18.0.
...
- Testing or debugging of implementations of the BitcoinII P2P network protocol
  should be done by inspecting the log messages that are produced by a recent
  version of BitcoinII Core. BitcoinII Core logs debug messages
...
- OpenSSL is no longer used by BitcoinII Core.  (#17265)

- BIP70 support has been fully removed from BitcoinII Core. The
...
- All custom settings configured when BitcoinII Core starts are now
...
- The "Start BitcoinII Core on system login" option has been removed on macOS.
  (#17567)
...
- A "known bug" [announced](https://bitcoincore.org/en/releases/0.18.0/#wallet-gui)
  ...
  affected anyone who simultaneously used multiple BitcoinII Core wallets
...
- BitcoinII Core's automatically-generated source code documentation is
...
- The `bitcoinII-cli` utility used with the `-getinfo` parameter now
...
- The `bitcoinII-cli` utility now accepts a `-stdinwalletpassphrase`
...
- #17068 Always generate `bitcoinIIstrings.cpp` on `make translate` (D4nte)
...
- #17741 Included `test_bitcoinII-qt` in msvc build (sipsorcery)
...
- #18702 Fix ASLR for bitcoinII-cli on Windows (fanquake)
...
- #17206 Add testcase to simulate bitcoinII schema in leveldb (adamjonas)
...
- #17330 Add `shrinkdebugfile=0` to regtest bitcoinII.conf (sdaftuar)
...
- #18022 Fix appveyor `test_bitcoinII` build of `*.raw` (MarcoFalke)
...
- #18166 ci: Run fuzz testing test cases (bitcoinII-core/qa-assets) under valgrind to catch memory errors (practicalswift)
...
- #18430 ci: Only clone bitcoinII-core/qa-assets when fuzzing (MarcoFalke)
...
- #16983 Add detailed info about BitcoinII Core files (hebasto)
...
- #17119 Fix broken bitcoinII-cli examples (andrewtoth)
...
- #17503 Remove bitness from bitcoinII-qt help message and manpage (laanwj)
...
- #17648 Rename wallet-tool references to bitcoinII-wallet (hel-o)
...
- #18341 Replace remaining literal BC2 with `CURRENCY_UNIT` (domob1812)
...
- #17650 util: Remove unwanted fields from bitcoinII-cli -getinfo (malevolent)
