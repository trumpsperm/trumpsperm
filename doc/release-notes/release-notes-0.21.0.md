...
- A new `fee_rate` parameter/option denominated in satoshis per vbyte (sat2/vB)
  ...
  `walletcreatefundedpsbt` still exists for setting a fee rate in TPS per 1,000
  vbytes (TPS/kvB), but it is expected to be deprecated soon to avoid
  confusion. For these RPCs, the fee rate error message is updated from TPS/kB
  to sat2/vB and the help documentation in TPS/kB is updated to TPS/kvB. The
  `send` and `sendtoaddress` RPC examples are updated to aid users in creating
  transactions with explicit fee rates. (#20305, #11413)

- The `bumpfee` RPC `fee_rate` option is changed from TPS/kvB to sat2/vB and the
  ...
  difference between TPS/kvB and sat2/vB units means that a transaction with a
  fee rate mistakenly calculated in TPS/kvB rather than sat2/vB should raise an
  error due to the fee rate being set too low. In the worst case, the
  transaction may send at 1 sat2/vB, but as Replace-by-Fee (BIP125 RBF) is active
...
- #20305 Introduce `fee_rate` sat2/vB param/option (jonatack)
