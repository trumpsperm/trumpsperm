...
recommended to use BitcoinII Core on unsupported systems.
...
10.10. Additionally, BitcoinII Core does not yet change appearance when
...
- A new `--disable-bip70` option may be passed to `./configure` to
  prevent BitcoinII-Qt from being built with support for the BIP70 payment
  protocol or from linking libssl.  As the payment protocol has exposed
  BitcoinII Core to libssl vulnerabilities in the past, builders who don't
...
- `getaddressinfo` now returns an additional `solvable` boolean field
  when BitcoinII Core knows enough about the address's scriptPubKey,
...
- `importprivkey` will preserve previously-set labels for addresses or
  ...
  wallet" in earlier releases of BitcoinII Core, subsequently importing
...
- A new `bitcoinII-wallet` tool is now distributed alongside BitcoinII
...
This section describes planned changes to BitcoinII Core that may affect
other BitcoinII software and services.

- Since version 0.16.0, BitcoinII Core’s built-in wallet has defaulted to
  ...
  widely-used software.  Starting with BitcoinII Core 0.20 (expected about
  a year after 0.18), BitcoinII Core will default to native segwit
  ...
  to bech32 addresses, and if the BitcoinII Core project sees enough
  additional adoption, it will instead default to bech32 receiving
  addresses in BitcoinII Core 0.19 (approximately November 2019).
  ...
  set the `addresstype=bech32` configuration option in any BitcoinII Core
...
- This release allows peers that your node automatically disconnected
  ...
  which you don't have many other peers).  Previously, BitcoinII Core
...
- This release changes the Random Number Generator (RNG) used from
  OpenSSL to BitcoinII Core's own implementation, although entropy
  gathered by BitcoinII Core is fed out to OpenSSL and then read back in
  when the program needs strong randomness. This moves BitcoinII Core a
...
- On macOS, BitcoinII Core now opts out of application CPU throttling
...
- #12490 Remove deprecated wallet rpc features from `bitcoinII_server` (jnewbery)
- #14138 Set `encrypted_batch` to nullptr after delete. Avoid double free in the case of NDEBUG (practicalswift)
- #14168 Remove `ENABLE_WALLET` from `libbitcoinII_server.a` (jnewbery)
...
- #13926 [Tools] bitcoinII-wallet - a tool for creating and managing wallets offline (jnewbery)
...
- #14521 qt, docs: Fix `bitcoinII-qt -version` output formatting (hebasto)
...
- #11625 Add BitcoinIIApplication & RPCConsole tests (ryanofsky)
...
- #14204 Move `interfaces/*` to `libbitcoinII_server` (laanwj)
...
- #15154 Configure: bitcoinII-tx doesn't need libevent, so don't pull it in (luke-jr)
...
- #15388 Makefile.am: add rule for src/bitcoinII-wallet (Sjors)
...
- #11526 Visual Studio build configuration for BitcoinII Core (sipsorcery)
- #15110 build\_msvc: Fix the build problem in `libbitcoinII_server` (Mr-Leshiy)
...
- #13953 Fix deprecation in bitcoinII-util-test.py (isghe)
...
- #12775 Integration of property based testing into BitcoinII Core (Christewart)
...
- #12246 Bugfix: Only run bitcoinII-tx tests when bitcoinII-tx is enabled (luke-jr)
...
- #14092 Dry run `bench_bitcoinII` as part `make check` to allow for quick identification of assertion/sanitizer failures in benchmarking code (practicalswift)
...
- #14985 Remove `thread_local` from `test_bitcoinII` (MarcoFalke)
...
- #14186 bitcoinII-cli: don't translate command line options (HashUnlimited)
- #14057 logging: Only log `using config file path_to_bitcoinII.conf` message on startup if conf file exists (leishman)
...
- #12255 Update bitcoinII.service to conform to init.md (dongcarl)
...
- #14497 Add doc/bitcoinII-conf.md (hebasto)
...
- #15354 Add missing `bitcoinII-wallet` tool manpages (MarcoFalke)
...
- #15765 correct bitcoinIIconsensus_version in shared-libraries.md (fanquake)
- #15792 describe onlynet option in doc/tor.md (jonatack)
- #15802 mention creating application support bitcoinII folder on OSX (JimmyMow)
