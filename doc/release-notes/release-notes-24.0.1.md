...
This version of BitcoinII Core adds a new `mempoolfullrbf` configuration
...
Some BitcoinII services today expect that the first version of an
...
The BitcoinII Protocol does not, and cannot, provide any assurance that
...
fees, was realized in [BitcoinII Core 0.12.0][] (February 2016) with the
...
default for years, and several contributors to BitcoinII Core are
...
- To help prevent fingerprinting transactions created by the BitcoinII Core wallet, change output
...
simplified. Settings from `bitcoinII.conf` are now displayed normally in the GUI
settings dialog, instead of in a separate warning message ("Options set in this
dialog are overridden by the configuration file: -setting=value"). And these
settings can now be edited because `settings.json` values take precedence over
`bitcoinII.conf` values. (#15936)
