BitcoinII Core version 0.9.0 is now available from:
...
Rebranding to BitcoinII Core
---------------------------

To reduce confusion between BitcoinII-the-network and BitcoinII-the-software we
have renamed the reference client to BitcoinII Core.
...
TX outputs, bloating bitcoinII's UTXO database.
...
0.1mBC2 per kilobyte. During periods of heavy transaction volume, even this
...
- Drop the fee required to relay a transaction to 0.01mBC2 per kilobyte
...
- Added new DNS seed from bitcoinIIstats.com
...
- OS X: Fix bitcoinII-qt startup crash when clicking dock icon
- Linux: Fix Gnome bitcoinII: URI handler
...
- Add separate bitcoinII-cli client
