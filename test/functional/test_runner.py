...
`test/functional/test_framework/test_framework.py:BitcoinIITestFramework.main`.
...
BASE_SCRIPTS = [
    ...
    'interface_bitcoinII_cli.py --legacy-wallet',
    'interface_bitcoinII_cli.py --descriptors',
...
def main():
    ...
    enable_bitcoinIId = config["components"].getboolean("ENABLE_BITCOIND")
...
def run_tests(*, test_list, build_dir, tmpdir, jobs=1, enable_coverage=False, args=None, combined_logs_len=0, failfast=False, use_term_control, results_filepath=None):
    ...
    if shutil.disk_usage(tmpdir).free < min_space:
        print(f"{BOLD[1]}WARNING!{BOLD[0]} There may be insufficient free space in {tmpdir} to run the BitcoinII functional test suite. "
    ...
    # a hard link or a copy on any platform. See https://github.com/bitcoinII/bitcoinII/pull/27561.
...
class RPCCoverage():
    ...
    commands per `bitcoinII-cli help` (`rpc_interface.txt`).
