...
class ToolWalletTest(BitcoinIITestFramework):
    ...
    def write_dump(self, dump, filename, magic=None, skip_checksum=False):
        if magic is None:
            magic = "BITCOINII_CORE_WALLET_DUMP"
    ...
    def test_dump_createfromdump(self):
        ...
        assert_equal(dump_data['BITCOINII_CORE_WALLET_DUMP'], '1')
        ...
        self.assert_raises_tool_error('Error: Dumpfile identifier record is incorrect. Got "not_the_right_magic", expected "BITCOINII_CORE_WALLET_DUMP".', '-wallet=badload', '-dumpfile={}'.format(bad_magic_wallet_dump), 'createfromdump')
        assert not (self.nodes[0].wallets_path / "badload").is_dir()
    ...
    def test_compare_legacy_dump_with_framework_bdb_parser(self):
        ...
        del expected_dump['BITCOINII_CORE_WALLET_DUMP']
