...
# The block reward of coinbaseoutput.nValue (50) TPS/block matures after
# COINBASE_MATURITY (100) blocks. Therefore, after mining 101 blocks we expect
# node 0 to have a balance of (BLOCKS - COINBASE_MATURITY) * 50 TPS/block.
...
WALLET_NOT_SPECIFIED = (
    ...
    "option before the command (run \"trumpsperm-cli -h\" for help or \"trumpsperm-cli listwallets\" to see "
...
class TestTrumpspermCli(TrumpspermTestFramework):
    ...
    def run_test(self):
        ...
        self.log.info("Compare responses from getblockchaininfo RPC and `trumpsperm-cli getblockchaininfo`")
        ...
        # Prevent trumpsperm-cli from using existing rpcport in conf
        ...
        self.log.info("Test -getinfo and trumpsperm-cli return all proxies")
        ...
        if self.is_specified_wallet_compiled():
            self.log.info("Test -getinfo and trumpsperm-cli getwalletinfo return expected wallet info")
            ...
            assert_equal(Decimal(cli_get_info['Transaction fee rate (-paytxfee) (TPS/kvB)']), wallet_info['paytxfee'])
            assert_equal(Decimal(cli_get_info['Min tx relay fee rate (TPS/kvB)']), network_info['relayfee'])
            ...
            # Mine a block to confirm; adds a block reward (50 TPS) to the default wallet.
            ...
            # Test trumpsperm-cli -generate with -rpcwallet in multiwallet mode.
...
if __name__ == '__main__':
    TestTrumpspermCli(__file__).main()
