...
const std::string TrumpspermGUI::DEFAULT_UIPLATFORM =
...
TrumpspermGUI::TrumpspermGUI(interfaces::Node& node, const PlatformStyle *_platformStyle, const NetworkStyle *networkStyle, QWidget *parent) :
    QMainWindow(parent),
    m_node(node),
    ...
    m_network_style(networkStyle)
{
    ...
    if(enableWallet)
    {
        ...
        connect(walletFrame, &WalletFrame::createWalletButtonClicked, this, &TrumpspermGUI::createWallet);
    ...
    connect(labelBlocksIcon, &GUIUtil::ClickableLabel::clicked, this, &TrumpspermGUI::showModalOverlay);
    connect(progressBar, &GUIUtil::ClickableProgressBar::clicked, this, &TrumpspermGUI::showModalOverlay);
...
void TrumpspermGUI::createActions()
{
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a Trumpsperm address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and trumpsperm: URIs)"));
    ...
    connect(overviewAction, &QAction::triggered, this, &TrumpspermGUI::gotoOverviewPage);
    connect(sendCoinsAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(sendCoinsAction, &QAction::triggered, [this]{ gotoSendCoinsPage(); });
    connect(receiveCoinsAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(receiveCoinsAction, &QAction::triggered, this, &TrumpspermGUI::gotoReceiveCoinsPage);
    connect(historyAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(historyAction, &QAction::triggered, this, &TrumpspermGUI::gotoHistoryPage);
#endif // ENABLE_WALLET
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your Trumpsperm addresses to prove you own them"));
    verifyMessageAction = new QAction(tr("&Verify message…"), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified Trumpsperm addresses"));
    m_load_psbt_action = new QAction(tr("&Load PSBT from file…"), this);
    m_load_psbt_action->setStatusTip(tr("Load Partially Signed Trumpsperm Transaction"));
    m_load_psbt_clipboard_action = new QAction(tr("Load PSBT from &clipboard…"), this);
    m_load_psbt_clipboard_action->setStatusTip(tr("Load Partially Signed Trumpsperm Transaction from clipboard"));
    ...
    openAction->setStatusTip(tr("Open a trumpsperm: URI"));
    ...
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible Trumpsperm command-line options").arg(CLIENT_NAME));
    ...
    connect(quitAction, &QAction::triggered, this, &TrumpspermGUI::quitRequested);
    connect(aboutAction, &QAction::triggered, this, &TrumpspermGUI::aboutClicked);
    connect(aboutQtAction, &QAction::triggered, qApp, QApplication::aboutQt);
    connect(optionsAction, &QAction::triggered, this, &TrumpspermGUI::optionsClicked);
    connect(showHelpMessageAction, &QAction::triggered, this, &TrumpspermGUI::showHelpMessageClicked);
    connect(openRPCConsoleAction, &QAction::triggered, this, &TrumpspermGUI::showDebugWindow);
    ...
    if(walletFrame)
    {
        ...
        connect(openAction, &QAction::triggered, this, &TrumpspermGUI::openClicked);
        connect(m_open_wallet_menu, &QMenu::aboutToShow, [this] {
            ...
            for (const auto& [path, info] : m_wallet_controller->listWalletDir()) {
                ...
                connect(action, &QAction::triggered, [this, path] {
                    ...
                    connect(activity, &OpenWalletActivity::opened, this, &TrumpspermGUI::setCurrentWallet, Qt::QueuedConnection);
        ...
        connect(m_restore_wallet_action, &QAction::triggered, [this] {
            ...
            connect(activity, &RestoreWalletActivity::restored, this, &TrumpspermGUI::setCurrentWallet, Qt::QueuedConnection);
        ...
        connect(m_create_wallet_action, &QAction::triggered, this, &TrumpspermGUI::createWallet);
        ...
        connect(m_migrate_wallet_menu, &QMenu::aboutToShow, [this] {
            ...
            for (const auto& [wallet_name, info] : m_wallet_controller->listWalletDir()) {
                ...
                connect(action, &QAction::triggered, [this, wallet_name] {
                    ...
                    connect(activity, &MigrateWalletActivity::migrated, this, &TrumpspermGUI::setCurrentWallet);
        ...
        connect(m_mask_values_action, &QAction::toggled, this, &TrumpspermGUI::setPrivacy);
        connect(m_mask_values_action, &QAction::toggled, this, &TrumpspermGUI::enableHistoryAction);
    }
#endif // ENABLE_WALLET

    connect(new QShortcut(QKeySequence(Qt::CTRL | Qt::SHIFT | Qt::Key_C), this), &QShortcut::activated, this, &TrumpspermGUI::showDebugWindowActivateConsole);
    connect(new QShortcut(QKeySequence(Qt::CTRL | Qt::SHIFT | Qt::Key_D), this), &QShortcut::activated, this, &TrumpspermGUI::showDebugWindow);
}

void TrumpspermGUI::createMenuBar()
{
    appMenuBar = menuBar();
...
void TrumpspermGUI::createToolBars()
{
    if(walletFrame)
    {
        ...
        connect(m_wallet_selector, qOverload<int>(&QComboBox::currentIndexChanged), this, &TrumpspermGUI::setCurrentWalletBySelectorIndex);
...
void TrumpspermGUI::setClientModel(ClientModel *_clientModel, interfaces::BlockAndHeaderTipInfo* tip_info)
{
    ...
    if(_clientModel)
    {
        ...
        connect(_clientModel, &ClientModel::numConnectionsChanged, this, &TrumpspermGUI::setNumConnections);
        connect(_clientModel, &ClientModel::networkActiveChanged, this, &TrumpspermGUI::setNetworkActive);
        ...
        connect(_clientModel, &ClientModel::numBlocksChanged, this, &TrumpspermGUI::setNumBlocks);
        ...
        connect(_clientModel, &ClientModel::showProgress, this, &TrumpspermGUI::showProgress);
...
#ifdef ENABLE_WALLET
void TrumpspermGUI::enableHistoryAction(bool privacy)
{
...
void TrumpspermGUI::setWalletController(WalletController* wallet_controller, bool show_loading_minimized)
{
    ...
    GUIUtil::ExceptionSafeConnect(wallet_controller, &WalletController::walletAdded, this, &TrumpspermGUI::addWallet);
    connect(wallet_controller, &WalletController::walletRemoved, this, &TrumpspermGUI::removeWallet);
...
WalletController* TrumpspermGUI::getWalletController()
{
    return m_wallet_controller;
}

void TrumpspermGUI::addWallet(WalletModel* walletModel)
{
    ...
    connect(wallet_view, &WalletView::outOfSyncWarningClicked, this, &TrumpspermGUI::showModalOverlay);
    connect(wallet_view, &WalletView::transactionClicked, this, &TrumpspermGUI::gotoHistoryPage);
    connect(wallet_view, &WalletView::coinsSent, this, &TrumpspermGUI::gotoHistoryPage);
    connect(wallet_view, &WalletView::message, [this](const QString& title, const QString& message, unsigned int style) {
        this->message(title, message, style);
    });
    connect(wallet_view, &WalletView::encryptionStatusChanged, this, &TrumpspermGUI::updateWalletStatus);
    connect(wallet_view, &WalletView::incomingTransaction, this, &TrumpspermGUI::incomingTransaction);
    connect(this, &TrumpspermGUI::setPrivacy, wallet_view, &WalletView::setPrivacy);
...
void TrumpspermGUI::removeWallet(WalletModel* walletModel)
{
...
void TrumpspermGUI::setCurrentWallet(WalletModel* wallet_model)
{
    if (!walletFrame || !m_wallet_controller) return;
...
void TrumpspermGUI::setCurrentWalletBySelectorIndex(int index)
{
    WalletModel* wallet_model = m_wallet_selector->itemData(index).value<WalletModel*>();
    if (wallet_model) setCurrentWallet(wallet_model);
}

void TrumpspermGUI::removeAllWallets()
{
    if(!walletFrame)
        return;
    setWalletActionsEnabled(false);
...
void TrumpspermGUI::setWalletActionsEnabled(bool enabled)
{
...
void TrumpspermGUI::createTrayIcon()
{
...
void TrumpspermGUI::createTrayIconMenu()
{
#ifndef Q_OS_MACOS
    if (!trayIcon) return;
#endif // Q_OS_MACOS
    ...
    show_hide_action = trayIconMenu->addAction(QString(), this, &TrumpspermGUI::toggleHidden);
...
void TrumpspermGUI::optionsClicked()
{
    openOptionsDialogWithTab(OptionsDialog::TAB_MAIN);
}

void TrumpspermGUI::aboutClicked()
{
    if(!clientModel)
        return;
...
void TrumpspermGUI::showDebugWindow()
{
    GUIUtil::bringToFront(rpcConsole);
    Q_EMIT consoleShown(rpcConsole);
}

void TrumpspermGUI::showDebugWindowActivateConsole()
{
    rpcConsole->setTabFocus(RPCConsole::TabTypes::CONSOLE);
    showDebugWindow();
}

void TrumpspermGUI::showHelpMessageClicked()
{
    GUIUtil::bringToFront(helpMessageDialog);
}

#ifdef ENABLE_WALLET
void TrumpspermGUI::openClicked()
{
...
void TrumpspermGUI::gotoOverviewPage()
{
    overviewAction->setChecked(true);
    if (walletFrame) walletFrame->gotoOverviewPage();
}

void TrumpspermGUI::gotoHistoryPage()
{
    historyAction->setChecked(true);
    if (walletFrame) walletFrame->gotoHistoryPage();
}

void TrumpspermGUI::gotoReceiveCoinsPage()
{
    receiveCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoReceiveCoinsPage();
}

void TrumpspermGUI::gotoSendCoinsPage(QString addr)
{
    sendCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoSendCoinsPage(addr);
}

void TrumpspermGUI::gotoSignMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoSignMessageTab(addr);
}

void TrumpspermGUI::gotoVerifyMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoVerifyMessageTab(addr);
}
void TrumpspermGUI::gotoLoadPSBT(bool from_clipboard)
{
    if (walletFrame) walletFrame->gotoLoadPSBT(from_clipboard);
}
#endif // ENABLE_WALLET

void TrumpspermGUI::updateNetworkState()
{
    ...
    if (m_node.getNetworkActive()) {
        ...
        tooltip = tr("%n active connection(s) to Trumpsperm network.", "", count);
    } else {
        //: A substring of the tooltip.
...
void TrumpspermGUI::setNumConnections(int count)
{
    updateNetworkState();
}

void TrumpspermGUI::setNetworkActive(bool network_active)
{
    updateNetworkState();
...
void TrumpspermGUI::updateHeadersSyncProgressLabel()
{
...
void TrumpspermGUI::updateHeadersPresyncProgressLabel(int64_t height, const QDateTime& blockDate)
{
...
void TrumpspermGUI::openOptionsDialogWithTab(OptionsDialog::Tab tab)
{
    if (!clientModel || !clientModel->getOptionsModel())
        return;

    auto dlg = new OptionsDialog(this, enableWallet);
    connect(dlg, &OptionsDialog::quitOnReset, this, &TrumpspermGUI::quitRequested);
...
void TrumpspermGUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, SyncType synctype, SynchronizationState sync_state)
{
...
void TrumpspermGUI::createWallet()
{
#ifdef ENABLE_WALLET
#ifndef USE_SQLITE
    ...
    connect(activity, &CreateWalletActivity::created, this, &TrumpspermGUI::setCurrentWallet);
    connect(activity, &CreateWalletActivity::created, rpcConsole, &RPCConsole::setCurrentWallet);
    activity->create();
#endif // ENABLE_WALLET
}

void TrumpspermGUI::message(const QString& title, QString message, unsigned int style, bool* ret, const QString& detailed_message)
{
...
void TrumpspermGUI::changeEvent(QEvent *e)
{
    ...
    if(e->type() == QEvent::WindowStateChange)
    {
        if(clientModel && clientModel->getOptionsModel() && clientModel->getOptionsModel()->getMinimizeToTray())
        {
            ...
            {
                QTimer::singleShot(0, this, &TrumpspermGUI::hide);
                e->ignore();
            }
            else if((wsevt->oldState() & Qt::WindowMinimized) && !isMinimized())
            {
                QTimer::singleShot(0, this, &TrumpspermGUI::show);
                e->ignore();
            }
        }
    }
...
void TrumpspermGUI::closeEvent(QCloseEvent *event)
{
#ifndef Q_OS_MACOS // Ignored on Mac
    if(clientModel && clientModel->getOptionsModel())
    {
...
void TrumpspermGUI::showEvent(QShowEvent *event)
{
...
#ifdef ENABLE_WALLET
void TrumpspermGUI::incomingTransaction(const QString& date, TrumpspermUnit unit, const CAmount& amount, const QString& type, const QString& address, const QString& label, const QString& walletName)
{
    // On new transaction, make an info balloon
    QString msg = tr("Date: %1\n").arg(date) +
                  tr("Amount: %1\n").arg(TrumpspermUnits::formatWithUnit(unit, amount, true));
...
void TrumpspermGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void TrumpspermGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
...
bool TrumpspermGUI::eventFilter(QObject *object, QEvent *event)
{
...
#ifdef ENABLE_WALLET
bool TrumpspermGUI::handlePaymentRequest(const SendCoinsRecipient& recipient)
{
    // URI has to be valid
...
void TrumpspermGUI::setHDStatus(bool privkeyDisabled, int hdEnabled)
{
...
void TrumpspermGUI::setEncryptionStatus(int status)
{
    switch(status)
    {
...
void TrumpspermGUI::updateWalletStatus()
{
    assert(walletFrame);
...
void TrumpspermGUI::updateProxyIcon()
{
    std::string ip_port;
...
void TrumpspermGUI::updateWindowTitle()
{
...
void TrumpspermGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if(!clientModel)
        return;
...
void TrumpspermGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void TrumpspermGUI::detectShutdown()
{
...
void TrumpspermGUI::showProgress(const QString &title, int nProgress)
{
    if (nProgress == 0) {
...
void TrumpspermGUI::showModalOverlay()
{
    if (modalOverlay && (progressBar->isVisible() || modalOverlay->isLayerVisible()))
        modalOverlay->toggleVisibility();
}

static bool ThreadSafeMessageBox(TrumpspermGUI* gui, const bilingual_str& message, const std::string& caption, unsigned int style)
{
    ...
    if (message.original != message.translated) {
        detailed_message = TrumpspermGUI::tr("Original message:") + "\n" + QString::fromStdString(message.original);
    }
...
void TrumpspermGUI::subscribeToCoreSignals()
{
...
void TrumpspermGUI::unsubscribeFromCoreSignals()
{
...
bool TrumpspermGUI::isPrivacyModeActivated() const
{
    ...
    : m_platform_style{platformStyle}
{
    ...
    QList<TrumpspermUnit> units = TrumpspermUnits::availableUnits();
    int max_width = 0;
    const QFontMetrics fm(font());
    for (const TrumpspermUnit unit : units) {
        max_width = qMax(max_width, GUIUtil::TextWidth(fm, TrumpspermUnits::longName(unit)));
    }
...
{
    menu = new QMenu(this);
    for (const TrumpspermUnit u : TrumpspermUnits::availableUnits()) {
        menu->addAction(TrumpspermUnits::longName(u))->setData(QVariant::fromValue(u));
    }
...
void UnitDisplayStatusBarControl::updateDisplayUnit(TrumpspermUnit newUnits)
{
    setText(TrumpspermUnits::longName(newUnits));
}
