...
static void RegisterMetaTypes()
{
    ...
    qRegisterMetaTypeStreamOperators<TrumpspermUnit>("TrumpspermUnit");
#else
    qRegisterMetaType<TrumpspermUnit>("TrumpspermUnit");
#endif
}
...
static void initTranslations(QTranslator &qtTranslatorBase, QTranslator &qtTranslator, QTranslator &translatorBase, QTranslator &translator)
{
    ...
    // Load e.g. trumpsperm_de.qm (shortcut "de" needs to be defined in trumpsperm.qrc)
    if (translatorBase.load(lang, ":/translations/")) {
        QApplication::installTranslator(&translatorBase);
    }

    // Load e.g. trumpsperm_de_DE.qm (shortcut "de_DE" needs to be defined in trumpsperm.qrc)
...
static const char* qt_argv = "trumpsperm-qt";

TrumpspermApplication::TrumpspermApplication()
...
void TrumpspermApplication::setupPlatformStyle()
{
    // UI per-platform customization
    // This must be done inside the TrumpspermApplication constructor, or after it, because
    // PlatformStyle::instantiate requires a QApplication
    std::string platformName;
    platformName = gArgs.GetArg("-uiplatform", TrumpspermGUI::DEFAULT_UIPLATFORM);
...
#ifdef ENABLE_WALLET
void TrumpspermApplication::createPaymentServer()
{
    paymentServer = new PaymentServer(this);
}
#endif

bool TrumpspermApplication::createOptionsModel(bool resetSettings)
{
...
void TrumpspermApplication::createWindow(const NetworkStyle *networkStyle)
{
    window = new TrumpspermGUI(node(), platformStyle, networkStyle, nullptr);
    connect(window, &TrumpspermGUI::quitRequested, this, &TrumpspermApplication::requestShutdown);
...
void TrumpspermApplication::createSplashScreen(const NetworkStyle *networkStyle)
{
    assert(!m_splash);
...
void TrumpspermApplication::createNode(interfaces::Init& init)
{
    assert(!m_node);
...
bool TrumpspermApplication::baseInitialize()
{
    return node().baseInitialize();
}

void TrumpspermApplication::startThread()
{
    assert(!m_executor);
    m_executor.emplace(node());

    /*  communication to and from thread */
    connect(&m_executor.value(), &InitExecutor::initializeResult, this, &TrumpspermApplication::initializeResult);
    connect(&m_executor.value(), &InitExecutor::shutdownResult, this, [] {
        QCoreApplication::exit(0);
    });
    connect(&m_executor.value(), &InitExecutor::runawayException, this, &TrumpspermApplication::handleRunawayException);
    connect(this, &TrumpspermApplication::requestedInitialize, &m_executor.value(), &InitExecutor::initialize);
    connect(this, &TrumpspermApplication::requestedShutdown, &m_executor.value(), &InitExecutor::shutdown);
}

void TrumpspermApplication::parameterSetup()
{
...
void TrumpspermApplication::InitPruneSetting(int64_t prune_MiB)
{
    optionsModel->SetPruneTargetGB(PruneMiBtoGB(prune_MiB));
}

void TrumpspermApplication::requestInitialize()
{
...
void TrumpspermApplication::requestShutdown()
{
...
void TrumpspermApplication::initializeResult(bool success, interfaces::BlockAndHeaderTipInfo tip_info)
{
    ...
    if (success) {
        ...
        // trumpsperm: URIs or payment requests:
        if (paymentServer) {
            connect(paymentServer, &PaymentServer::receivedPaymentRequest, window, &TrumpspermGUI::handlePaymentRequest);
            connect(window, &TrumpspermGUI::receivedURI, paymentServer, &PaymentServer::handleURIOrFile);
...
void TrumpspermApplication::handleRunawayException(const QString &message)
{
...
void TrumpspermApplication::handleNonFatalException(const QString& message)
{
...
WId TrumpspermApplication::getMainWinId() const
{
    if (!window)
        return 0;
...
bool TrumpspermApplication::event(QEvent* e)
{
...
static void SetupUIArgs(ArgsManager& argsman)
{
    ...
    argsman.AddArg("-uiplatform", strprintf("Select platform to customize UI for (one of windows, macosx, other; default: %s)", TrumpspermGUI::DEFAULT_UIPLATFORM), ArgsManager::ALLOW_ANY | ArgsManager::DEBUG_ONLY, OptionsCategory::GUI);
}
...
int GuiMain(int argc, char* argv[])
{
    ...
    Q_INIT_RESOURCE(trumpsperm);
    Q_INIT_RESOURCE(trumpsperm_locale);
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (invalid_token) {
            InitError(Untranslated(strprintf("Command line contains unexpected token '%s', see trumpsperm-qt -h for a list of options.", argv[i])));
            QMessageBox::critical(nullptr, CLIENT_NAME,
                                  // message cannot be translated because translations have not been initialized
                                  QString::fromStdString("Command line contains unexpected token '%1', see trumpsperm-qt -h for a list of options.").arg(QString::fromStdString(argv[i])));
            return EXIT_FAILURE;
        }
    }
    ...
    /// 6-7. Parse trumpsperm.conf, determine network, switch to network specific
    ...
    // trumpsperm: links repeatedly have their payment requests routed to this process:
    if (WalletModel::isWalletEnabled()) {
