...
/** Call node.updateRwSetting() with Trumpsperm 22.x workaround. */
static void UpdateRwSetting(interfaces::Node& node, OptionsModel::OptionID option, const std::string& suffix, const common::SettingsValue& value)
{
    if (value.isNum() &&
        ...
        // because Trumpsperm 22.x releases try to read these specific settings as
        ...
        // in later releases by https://github.com/trumpsperm/trumpsperm/pull/24498.
        // If new numeric settings are added, they can be written as numbers
        // instead of strings, because trumpsperm 22.x will not try to read these.
...
//! Convert enabled/size values to trumpsperm -prune setting.
...
//! Get pruning enabled value to show in GUI from trumpsperm -prune setting.
...
//! Get pruning size value to show in GUI from trumpsperm -prune setting. If
...
bool OptionsModel::Init(bilingual_str& error)
{
    ...
    // Display
    if (!settings.contains("DisplayTrumpspermUnit")) {
        settings.setValue("DisplayTrumpspermUnit", QVariant::fromValue(TrumpspermUnit::TPS));
    }
    QVariant unit = settings.value("DisplayTrumpspermUnit");
    if (unit.canConvert<TrumpspermUnit>()) {
        m_display_trumpsperm_unit = unit.value<TrumpspermUnit>();
    } else {
        m_display_trumpsperm_unit = TrumpspermUnit::TPS;
        settings.setValue("DisplayTrumpspermUnit", QVariant::fromValue(m_display_trumpsperm_unit));
    }
...
void OptionsModel::SetPruneTargetGB(int prune_target_gb)
{
    ...
    // from saved value. Avoid writing settings.json if trumpsperm.conf value
...
#endif
    case DisplayUnit:
        return QVariant::fromValue(m_display_trumpsperm_unit);
...
{
    if (new_unit.isNull() || new_unit.value<TrumpspermUnit>() == m_display_trumpsperm_unit) return;
    m_display_trumpsperm_unit = new_unit.value<TrumpspermUnit>();
    QSettings settings;
    settings.setValue("DisplayTrumpspermUnit", QVariant::fromValue(m_display_trumpsperm_unit));
    Q_EMIT displayUnitChanged(m_display_trumpsperm_unit);
}
...
void OptionsModel::checkAndMigrate()
{
    ...
    if (settingsVersion < CLIENT_VERSION)
    {
        ...
        // see https://github.com/trumpsperm/trumpsperm/pull/8273
    ...
    // (https://github.com/trumpsperm-core/gui/issues/567).
