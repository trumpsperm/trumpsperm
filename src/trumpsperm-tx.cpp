...
static void SetupTrumpspermTxArgs(ArgsManager &argsman)
{
...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(gArgs) || gArgs.GetBoolArg("-version", false)) {
        ...
        std::string strUsage = CLIENT_NAME " trumpsperm-tx utility version " + FormatFullVersion() + "\n";

        if (gArgs.GetBoolArg("-version", false)) {
            strUsage += FormatParagraph(LicenseInfo());
        } else {
            strUsage += "\n"
                "The trumpsperm-tx tool is used for creating and modifying trumpsperm transactions.\n\n"
                "trumpsperm-tx can be used with \"<hex-tx> [commands]\" to update a hex-encoded trumpsperm transaction, or with \"-create [commands]\" to create a hex-encoded trumpsperm transaction.\n"
                "\n"
                "Usage: trumpsperm-tx [options] <hex-tx> [commands]\n"
                "or:    trumpsperm-tx [options] -create [commands]\n"
                "\n";
...
static int CommandLineRawTx(int argc, char* argv[])
{
    ...
    try {
        ...
        if (!fCreateBlank) {
            ...
            // param: hex-encoded trumpsperm transaction
